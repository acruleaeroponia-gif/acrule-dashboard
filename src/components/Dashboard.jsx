...
export default function Dashboard({ ventas, egresos, config, lastUpdated, stale, error, onOpenConfig }) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex bg-slate-800 rounded-xl p-1 border border-slate-700">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                periodo === 'dia' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
              Hoy
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                periodo === 'semana' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
              Esta semana
            </button>
          ...
          <button
            ...
            className="bg-slate-800 border border-slate-700 text-slate-400 hover:text-white rounded-xl p-2 transition-colors"
            title="Configuración"
          >
            ⚙️
          </button>
      ...
      {error && (
        <div className="mb-4 bg-slate-800 border border-yellow-600 rounded-xl p-3 text-yellow-300 text-sm">
          {error}
        </div>
      )}

      {/* Alerta roja si hay overspend */}
      <div className="mb-6">
        <AlertaBanner diferencia={diferencia} modelosDesconocidos={modelosDesconocidos} />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 mb-6">
        <MetricCard
          label="Cobrado hoy"
          value={cobradoTotal}
          subtitle={`${metricas.cantidadVentas} venta${metricas.cantidadVentas !== 1 ? 's' : ''}`}
        />
        <MetricCard
          label="Pago a inversores"
          value={pagoInversoresTotal}
          color="white"
          subtitle="Costo de huertas"
        />
        <MetricCard
          label="Disponible para ads"
          value={disponibleAds}
          color={disponibleColor}
          subtitle={`Burn fijos: ${formatARS(burnFijos)}`}
        />
        <MetricCard
          label="Ads gastados"
          value={adsGastados}
          color={diferencia < 0 ? 'red' : 'white'}
          subtitle={diferencia >= 0
            ? `Margen: ${formatARS(diferencia)}`
            : `Exceso: ${formatARS(Math.abs(diferencia))}`}
        />
        <MetricCard
          label="EBITDA del día"
          value={ebitda}
          color={ebitdaColor}
          subtitle={ebitda >= 0 ? '✅ Positivo' : '❌ Negativo'}
          className="col-span-2 sm:col-span-1"
        />
      </div>

      {/* Acumulado semanal extra: transferencia a inversores */}
      {periodo === 'semana' && (
      ...
      <p className="text-center text-slate-600 text-xs mt-6">
        Refresco automático cada 5 minutos · Acrule Hidropónica 2026
      </p>
    </div>
  )
}
